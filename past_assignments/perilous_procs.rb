...
def selected_map!(arr, prc_1, prc_2)
    arr.each.with_index do |el, i|
        arr[i] = prc_2.call(el) if prc_1.call(el)
    end
...
def chain_map(val, arr)
    final_val = val
    arr.each { |prc| final_val = prc.call(final_val) }
    final_val
end
...
def proc_suffix(sentence, hash)
    ...
    (0...words.length).each do |i|
        suffix = ""
        hash.each_key do |proc|
            suffix += hash[proc] if proc.call(words[i])
        end
        new_sentence += words[i] + suffix + " "
    end
...
def proctition_platinum(arr, *prcs)
    ...
    arr.each do |el|
        prcs.each.with_index do |prc, i|
            if prc.call(el) && !visited.include?(el)
                    new_hash[i+1] << el
                    visited << el
            end
        end
